...
class CommandMessage {
	...
	parseArgs() {
		switch(this.command.argsType) {
			...
			default:
				throw new RangeError(`ไม่เข้าใจ  .. "${this.argsType}".`);
		}
	}
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return await this.reply(`The \`${this.command.name}\` ต้องใช้คำสั่งที่ channel เท่านั้นนะ `);
		}
		if(!this.command.hasPermission(this)) {
			this.client.emit('commandBlocked', this, 'permission');
			return await this.reply(`คุณไม่มีสิทธิ์ในการออกคำสั่ง \`${this.command.name}\`นะ  !!`);
		}
		...
		if(throttle) {
			...
			if(throttle.usages > this.command.throttling.usages) {
				...
				return await this.reply(
					`ไม่สามารถใช้คำสั่ง  \`${this.command.name}\` ใช้ได้อีกครั้งเมื่อเวลาผ่านไป ${remaining.toFixed(1)} วินาที`
				);
			}
		}
		...
		if(!args && this.command.args) {
			...
			if(!args) return await this.reply('ยกเลิกคำสั่ง !!');
		...
		try {
			this.client.emit('debug', `คำสั่ง ${this.command.groupID} ทำงาน :${this.command.memberName}.`);
