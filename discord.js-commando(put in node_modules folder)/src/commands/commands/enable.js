...
module.exports = class EnableCommandCommand extends Command {
			...
			args: [
				{
					key: 'cmdOrGrp',
					label: 'command/group',
					prompt: 'อยากจะเปิดการใช้งานคำสั่งอะไร ?',
					validate: val => {
						if(!val) return false;
	...
	async run(msg, args) {
		if(args.cmdOrGrp.isEnabledIn(msg.guild)) {
			return msg.reply(
				`คำสั่ง  \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? '' : ''} ถูกเปิดใช้งานอยู่แล้วนี่น่า `
			);
		}
		args.cmdOrGrp.setEnabledIn(msg.guild, true);
		return msg.reply(`เปิดการใช้งานคำสั่ง  \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? '' : 'เรียบร้อยแล้ว  '}`);
	}
};
